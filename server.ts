  ...
  app.get("/api/services", async (req, res) => {
    ...
    try {
      ...
      if (filter.status === 'approved' && !createdBy) {
        ...
        // 1. Add one random sponsored service
        if (sponsored.length > 0) {
          // Use a seed based on page and date (optional, but let's keep it simple and random for now as requested)
          // Actually, to keep it consistent during a single session/page load, we can use a seed.
          // But "Randomized at each page" could mean every time you load the page.
          const randomIndex = Math.floor(Math.random() * sponsored.length);
          pageItems.push({ type: 'service', data: sponsored[randomIndex] });
        }
