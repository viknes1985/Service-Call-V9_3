...
async function startServer() {
  ...
  app.post("/api/auth/signup", async (req, res) => {
    const { email, code, password, firstName, lastName, mobileNumber } = req.body;
    const id = Math.random().toString(36).substring(2, 15);
    try {
      // In a real app, we'd verify the code here. 
      // For simplicity, let's assume the code was sent and we check it now.
      // We need to store the code somewhere. I'll use a temporary collection or just the User collection.
      ...
      // If we used the "send-verification" endpoint above, we'd have a user with isVerified: false
      // Let's refine the signup flow:
      // 1. Client calls /api/auth/send-verification (creates user with isVerified: false or updates)
      // 2. Client calls /api/auth/signup with the code.
      ...
      res.json({ id: user._id, email, firstName, lastName, mobileNumber });
    } catch (err: any) {
      res.status(400).json({ error: err.message });
    }
  });
    ...
    try {
      ...
      if (!user) {
        user = new User({
          ...
          email,
          password: "TEMP_PASSWORD", // Will be overwritten
          isVerified: false,
