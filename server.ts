...
async function startServer() {
  ...
  app.delete("/api/admin/users/:id", async (req, res) => {
    ...
    try {
      // Also delete their services and ratings?
      // For now just delete the user
      const deleted = await User.findByIdAndDelete(id);
      if (!deleted) return res.status(404).json({ error: "User not found" });
      
      // Optional: delete services created by this user
      await Service.deleteMany({ createdBy: id });
      // Optional: delete ratings by this user
  ...
  app.get("/api/services", async (req, res) => {
    ...
    try {
      ...
      // Also allow admins to see sponsors in the Find tab (where status is not explicitly provided)
      let result: any[] = [];
      if ((filter.status === 'approved' || (isAdmin === 'true' && !status)) && !createdBy) {
        ...
        if (activeSponsors.length > 0) {
          ...
          // Even if pageServices is empty (0 results), we show the sponsor if it's page 1
          if (pageServices.length > 0 || currentPage === 1) {
        ...
        res.json({
          ...
          total: totalServices, // Frontend uses pages for pagination
