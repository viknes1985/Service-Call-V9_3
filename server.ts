...
const ServiceSchema = new mongoose.Schema({
  ...
  type: { type: String, enum: ['Service Provider', 'Consumer', 'Admin'], default: 'Service Provider' },
  isVerified: { type: Boolean, default: false }
}, { _id: false });
...
async function startServer() {
  ...
  app.post("/api/upload-image", async (req, res) => {
    ...
    try {
      const url = await saveToImgBB(image);
      if (url) {
        res.json({ url });
      } else {
  ...
  app.get("/api/admin/users", async (req, res) => {
    const { adminEmail, search, page = 1, limit = 40 } = req.query;
  ...
  app.patch("/api/admin/users/:id", async (req, res) => {
    ...
    try {
      const updated = await User.findByIdAndUpdate(id, { firstName, lastName, mobileNumber, email }, { new: true });
      res.json(updated);
    } catch (err: any) {
      res.status(400).json({ error: err.message });
    }
  ...
  app.delete("/api/admin/users/:id", async (req, res) => {
    ...
    try {
      ...
      // Also delete their services? User didn't specify, but usually safer to keep or mark inactive.
  ...
  app.get("/api/services", async (req, res) => {
    const { state, town, category, search, createdBy, status, isAdmin, minRating, maxRating, page = 1, limit = 20 } = req.query;
    let filter: any = {};
    ...
    try {
      ...
      if (filter.status === 'approved' && !createdBy) {
        ...
        // Now interleave Sponsor objects (from Sponsor collection)
        // 1 sponsor card per page (max 10 cards total per page)
        // So we need to return 9 services + 1 sponsor per page
        const activeSponsors = await Sponsor.find({ isEnabled: true }).sort({ createdAt: 1 });
        
        // We'll handle the 1-sponsor-per-page logic during pagination
        // For now, just prepare the services list
        const servicesOnly = tempResult.map(s => ({ type: 'service', data: s }));
        
        // Pagination
        const itemsPerPage = 9; // 9 services + 1 sponsor = 10 total
        const totalServices = servicesOnly.length;
        const totalPages = Math.ceil(totalServices / itemsPerPage);
        const currentPage = Number(page);
        const skip = (currentPage - 1) * itemsPerPage;
        const pageServices = servicesOnly.slice(skip, skip + itemsPerPage);

        if (activeSponsors.length > 0 && pageServices.length > 0) {
          // Pick a sponsor for this page (cycle through them)
          const sponsor = activeSponsors[(currentPage - 1) % activeSponsors.length];
          // Insert at random position (0 to pageServices.length)
          // We use a stable seed based on page number so it doesn't jump on every re-fetch
          const seed = currentPage * 12345;
          const randomPos = Math.floor((Math.abs(Math.sin(seed)) * 1000) % (pageServices.length + 1));
          pageServices.splice(randomPos, 0, { type: 'sponsor', data: sponsor });
        }

        res.json({
          services: pageServices,
          total: totalServices + totalPages, // Approximate total including sponsors
          pages: totalPages
        });
        return;
      } else {
        result = filtered.map(s => ({ type: 'service', data: s }));
      }
  ...
  app.post("/api/services", async (req, res) => {
    ...
    try {
      ...
      const newService = new Service({
        ...
        status: 'pending',
        type: isServiceAdmin ? 'Admin' : (req.body.type || 'Service Provider')
      });
            ...
            html: `
              <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
                ...
                <div style="background: #f9fafb; padding: 15px; border-radius: 8px;">
                  ...
                  <p><strong>Type:</strong> ${newService.type}</p>
                </div>
