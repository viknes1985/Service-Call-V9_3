...
import { Search, Plus, User, LogIn, LogOut, ChevronRight, MapPin, Clock, Phone, Camera, X, Filter, ChevronLeft, Trash2, Wrench, PhoneCall, Star, Check, Shield, AlertCircle, Menu, Info, Mail, Eye, EyeOff, Globe, Edit, Layout, PlusCircle, Loader2 } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import BottomNav from './components/BottomNav';
import { Category, Service, UserProfile, Sponsor, Rating } from './types';
import { CATEGORIES, STATES, MALAYSIA_STATES } from './constants';
import { auth, db } from './firebase';
import { onAuthStateChanged, signInWithPopup, GoogleAuthProvider, FacebookAuthProvider, signOut, sendPasswordResetEmail, updatePassword, reauthenticateWithCredential, EmailAuthProvider } from 'firebase/auth';
import { collection, doc, getDoc, getDocs, setDoc, updateDoc, deleteDoc, query, where, orderBy, limit, onSnapshot, Timestamp, getDocFromServer, startAfter, count, getCountFromServer } from 'firebase/firestore';
    ...
    if (this.state.hasError) {
      return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 p-8 rounded-3xl max-w-md w-full shadow-2xl space-y-4">
            ...
            <h2 className="text-xl font-bold dark:text-white">Something went wrong</h2>
            <p className="text-gray-600 dark:text-gray-400 text-sm">
              An error occurred in the application. Please try refreshing the page.
            </p>
            <div className="p-3 bg-gray-100 dark:bg-gray-800 rounded-xl overflow-auto max-h-32">
...
export default function App() {
  ...
  const [showWelcomeOverlay, setShowWelcomeOverlay] = useState(!localStorage.getItem('welcomeShown'));
  const [showConsolidatedGuide, setShowConsolidatedGuide] = useState(false);
  ...
  // Onboarding Guides
  const [showConsolidatedGuideState, setShowConsolidatedGuideState] = useState(false);
      ...
      if (firebaseUser) {
        try {
          ...
          if (userDoc.exists()) {
            ...
            // This might happen if user is authenticated but profile not created yet
            // (e.g. during signup process)
            // We'll handle this in the signup/login flow
          }
        } catch (error) {
  ...
  }, [isAuthReady, findCurrentPage, searchQuery, filterState, filterTown, filterCategory, minRating, maxRating]);

  useEffect(() => {
    if (isAuthReady && user && activeTab === 'profile') {
      fetchUserServices(1);
    }
  }, [isAuthReady, user, activeTab, adminTab, adminMainTab]);
  ...
  useEffect(() => {
    if (isAuthReady && isAdmin && user && activeTab === 'profile' && adminMainTab === 'users') {
      fetchAdminUsers(1, adminUsersSearch);
    }
  }, [isAuthReady, isAdmin, user, activeTab, adminMainTab]);

  const fetchServices = async (page = 1) => {
    setIsLoading(true);
    try {
      const url = `/api/services?page=${page}&limit=10&state=${encodeURIComponent(filterState)}&town=${encodeURIComponent(filterTown)}&category=${encodeURIComponent(filterCategory)}&search=${encodeURIComponent(searchQuery)}&isAdmin=${isAdmin}&minRating=${minRating}&maxRating=${maxRating}`;
      ...
      if (res.ok) {
        ...
        setFindCurrentPage(page);
      } else {
        showToast(data.error || 'Failed to fetch services', 'error');
      }
    } catch (error) {
  ...
  const fetchUserServices = async (page = 1) => {
    ...
    try {
      ...
      const results = snapshot.docs.map(doc => ({ ...(doc.data() as any), id: doc.id } as Service));
      setUserServices(results);
      
      if (isAdmin) {
        setAdminServicesTotal(results.length);
        setAdminServicesPages(1);
  ...
  const fetchTopCategories = async () => {
    try {
      // In a real app, we might aggregate this. For now, we'll just use the constants
      // or fetch from a 'categories' collection if it exists.
      // Let's assume we have a categories collection with 'count' field.
      const snapshot = await getDocs(query(collection(db, 'services'), where('status', '==', 'approved')));
  ...
  const fetchAdminUsers = async (page = 1, search = '') => {
    ...
    try {
      ...
      const snapshot = await getDocs(q);
      let results = snapshot.docs.map(doc => ({ ...(doc.data() as any), id: doc.id } as UserProfile));
      ...
      setAdminUsersTotal(results.length);
      setAdminUsersPages(1);
  ...
  const fetchSponsors = async () => {
    try {
      let q;
      if (isAdmin) {
        q = query(collection(db, 'sponsors'), orderBy('createdAt', 'desc'));
      } else {
        q = query(collection(db, 'sponsors'), where('isEnabled', '==', true));
      }
      const snapshot = await getDocs(q);
      const results = snapshot.docs.map(doc => ({ ...(doc.data() as any), id: doc.id } as Sponsor));
      setSponsors(results);
    } catch (error) {
  ...
  return (
    <div className={`min-h-screen pb-20 font-sans transition-colors duration-300 ${isDarkMode ? 'dark bg-black text-white' : 'bg-white text-gray-950'}`}>
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="flex items-center gap-1 overflow-hidden">
                                <div className="flex items-center gap-1 flex-shrink-0">
                                  ...
                                  ) : item.data.type === 'Consumer' ? (
                                    <div className="w-4 h-4 rounded-full bg-emerald-500 flex items-center justify-center text-[10px] font-bold text-white shadow-sm" title="Consumer">C</div>
                                  ) : (
                            ...
                            {adminUsers.length > 0 ? (
                              <table className="w-full text-left text-[11px]">
                                <thead className={`sticky top-0 z-10 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
                                  <tr>
                                    <th className={`p-3 font-bold uppercase tracking-wider text-[9px] ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>#</th>
                                    <th className={`p-3 font-bold uppercase tracking-wider text-[9px] ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>Name</th>
                                    <th className={`p-3 font-bold uppercase tracking-wider text-[9px] ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>Phone</th>
                                    <th className={`p-3 font-bold uppercase tracking-wider text-[9px] ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>Email</th>
                                    <th className={`p-3 font-bold uppercase tracking-wider text-[9px] text-right ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>Actions</th>
                                  </tr>
                                </thead>
                                  ...
                                  {adminUsers.map((u, i) => (
                                    <tr key={u.id} className={`${isDarkMode ? 'hover:bg-gray-700/50' : 'hover:bg-gray-50 bg-white'} transition-colors`}>
                                      ...
                                      <td className="p-3 text-right space-x-2">
                                        ...
                                        <button 
                                          onClick={async () => {
                                            if (confirm(`Are you sure you want to delete user ${u.firstName}?`)) {
                                              try {
                                                const res = await fetch(`/api/admin/users/${u.id}?adminEmail=${encodeURIComponent(user?.email || '')}`, { method: 'DELETE' });
                                                if (res.ok) {
      ...
      <AnimatePresence>
        {showConsolidatedGuide && (
          <ConsolidatedGuide 
            isDarkMode={isDarkMode}
            onClose={() => {
              setShowConsolidatedGuide(false);
              localStorage.setItem('hasSeenConsolidatedGuide', 'true');
            }} 
          />
        )}
      </AnimatePresence>
...
function SponsorForm({ isDarkMode, onSuccess, user, sponsorToEdit, onCancel, showToast }: { 
  isDarkMode: boolean, 
  onSuccess: () => void, 
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      const remaining = 2 - existingPhotos.length;
      if (remaining <= 0) {
        showToast('Maximum 2 photos allowed. Remove existing photos first.', 'error');
        return;
      }
      const selectedFiles = Array.from(e.target.files);
      const newPhotos = selectedFiles.slice(0, remaining);
      
      if (selectedFiles.length > remaining) {
        showToast(`Only ${remaining} more photo(s) can be added.`, 'error');
      }
      
      setPhotos(prev => [...prev, ...newPhotos].slice(0, remaining));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sponsorData = {
        ...
        isEnabled: sponsorToEdit ? sponsorToEdit.isEnabled : true,
        expiresAt: sponsorToEdit ? sponsorToEdit.expiresAt : Date.now() + (formData.durationDays * 24 * 60 * 60 * 1000),
        createdAt: sponsorToEdit ? sponsorToEdit.createdAt : Date.now()
      };

      if (sponsorToEdit) {
        await updateDoc(doc(db, 'sponsors', sponsorToEdit.id), sponsorData);
      } else {
        const id = Math.random().toString(36).substring(2, 15);
        await setDoc(doc(db, 'sponsors', id), { ...sponsorData, id });
      }
      ...
      onSuccess();
      showToast(sponsorToEdit ? 'Sponsor updated successfully!' : 'Sponsor added successfully!');
    } catch (err) {
      console.error(err);
      handleFirestoreError(err, sponsorToEdit ? OperationType.UPDATE : OperationType.CREATE, 'sponsors');
...
function SponsorList({ sponsors, isDarkMode, onUpdate, user, onEdit, showToast }: { 
  sponsors: Sponsor[], 
  isDarkMode: boolean, 
  onUpdate: () => void, 
  user: UserProfile | null,
  ...
  const toggleSponsor = async (id: string, isEnabled: boolean) => {
    try {
      await updateDoc(doc(db, 'sponsors', id), { isEnabled });
      onUpdate();
      showToast(`Sponsor ${isEnabled ? 'enabled' : 'disabled'} successfully`);
    } catch (err) {
      handleFirestoreError(err, OperationType.UPDATE, `sponsors/${id}`);
      showToast('Failed to update sponsor status', 'error');
    }
  };

  const updateDuration = async (id: string, sponsor: Sponsor) => {
    try {
      const newExpiresAt = Date.now() + (newDuration * 24 * 60 * 60 * 1000);
      ...
      onUpdate();
      showToast('Sponsor duration updated successfully');
    } catch (err) {
      handleFirestoreError(err, OperationType.UPDATE, `sponsors/${id}`);
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button 
                    ...
                    className="text-red-500 p-1 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg disabled:opacity-50"
                  >
              ...
              <div className="flex items-center justify-between gap-2">
                {editingDuration === s.id ? (
                  <div className="flex items-center gap-2 w-full">
                    ...
                    />
                    <button onClick={() => updateDuration(s.id, s)} className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-lg font-bold">Save</button>
        ...
        {confirmDeleteId && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              className={`w-full max-w-sm rounded-3xl p-6 shadow-2xl ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}
            >
              <div className="flex flex-col items-center text-center gap-4">
                ...
                <div className="space-y-2">
                  <h3 className="text-xl font-bold">Delete Sponsor?</h3>
                ...
                <div className="flex gap-3 w-full mt-2">
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl font-bold active:scale-95 transition-transform ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}
                  >
                    Cancel
                  </button>
                  <button
                    onClick={() => deleteSponsor(confirmDeleteId)}
                    disabled={deletingId === confirmDeleteId}
                    className="flex-1 py-3 rounded-xl font-bold bg-red-600 text-white active:scale-95 transition-transform disabled:opacity-50 flex items-center justify-center gap-2"
                  >
  ...
  const handleSocialLogin = async (providerName: 'google' | 'facebook') => {
    try {
      ...
      if (userDoc.exists()) {
        ...
        // Create new profile for social user
      ...
      if (error.code === 'auth/popup-blocked') {
        ...
        setError(error.message || `Failed to login with ${providerName}`);
      }
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div className="relative flex items-center">
                ...
                <span className="flex-shrink mx-4 text-gray-400 text-[10px] font-bold uppercase">Or login with</span>
                <div className="flex-grow border-t border-gray-200 dark:border-gray-700"></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`flex items-center justify-center gap-2 p-3 rounded-xl border transition-all active:scale-95 ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:bg-gray-700' : 'bg-white border-gray-200 hover:bg-gray-50'}`}
                >
        ...
        {mode === 'signup' && (
          <div className="space-y-3 pt-2">
            <div className="relative flex items-center">
              <div className="flex-grow border-t border-gray-200 dark:border-gray-700"></div>
              <span className="flex-shrink mx-4 text-gray-400 text-[10px] font-bold uppercase">Or sign up with</span>
              <div className="flex-grow border-t border-gray-200 dark:border-gray-700"></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <button
...
function NewServiceModal({ user, serviceToEdit, onClose, onSuccess, isDarkMode }: { user: UserProfile | null, serviceToEdit?: Service | null, onClose: () => void, onSuccess: (isNew: boolean) => void, isDarkMode: boolean }) {
  ...
  const [formData, setFormData] = useState({
    ...
    contactNumber: '',
    type: serviceToEdit?.type || 'Provider'
  });
  const [countryCode, setCountryCode] = useState('60');
  ...
  return (
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <label className={`text-xs font-bold uppercase tracking-wider ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Service Type</label>
            <div className="flex gap-2">
              <button
                type="button"
              ...
              <button
                type="button"
                onClick={() => setFormData(prev => ({ ...prev, type: 'Consumer' }))}
                className={`flex-1 py-3 rounded-xl text-sm font-bold border-2 transition-all ${
...
function UserEditModal({ user, isDarkMode, onClose, onUpdate, isUpdating }: { user: UserProfile, isDarkMode: boolean, onClose: () => void, onUpdate: (data: any) => Promise<void>, isUpdating: boolean }) {
  const [formData, setFormData] = useState({
  ...
  return (
    ...
    >
      <motion.div
        initial={{ scale: 0.9, y: 20 }}
        animate={{ scale: 1, y: 0 }}
        className={`${isDarkMode ? 'bg-gray-800 text-white border border-gray-700' : 'bg-white text-gray-900'} w-full max-w-md rounded-3xl p-6 space-y-6 shadow-2xl relative`}
      >
        <button onClick={onClose} className="absolute top-4 right-4 p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full">
          <X size={20} />
        </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <input
                type="text"
                required
                className={`w-full p-3 rounded-xl text-sm outline-none focus:ring-1 focus:ring-blue-500 transition-colors ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}
                value={formData.firstName}
                onChange={(e) => setFormData({ ...formData, firstName: e.target.value })}
              />
            </div>
          ...
          <button
            type="submit"
            disabled={isUpdating}
            className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold shadow-lg shadow-blue-500/20 active:scale-95 transition-transform disabled:opacity-50"
          >
...
function ConsolidatedGuide({ isDarkMode, onClose }: { isDarkMode: boolean, onClose: () => void }) {
  const [step, setStep] = useState(0);
  const steps = [
    {
      title: "Welcome to JomPro",
      icon: <Layout size={32} className="text-blue-500" />,
      content: "Your all-in-one platform for local services. Let's show you how to get the most out of it.",
      sections: [
        { label: "Find Service", desc: "Browse and filter local experts" },
        { label: "New Service", desc: "Register your own business" }
      ]
    },
    {
      title: "Finding Services",
      icon: <Search size={32} className="text-emerald-500" />,
      content: "Use the Find tab to search for specific services in your area.",
      sections: [
        { label: "Filters", desc: "Filter by State, Town, and Category" },
        { label: "Ratings", desc: "See what others are saying" }
      ]
    },
    {
      title: "Rating & Reviews",
      icon: <Star size={32} className="text-yellow-500" />,
      content: "Help the community by rating services you've used.",
      sections: [
        { label: "Stars", desc: "Select 1-5 stars for quality" },
        { label: "Comments", desc: "Add a short review (max 20 words)" }
      ]
    },
    {
      title: "Service Registration",
      icon: <PlusCircle size={32} className="text-purple-500" />,
      content: "Register as a Provider or Consumer to reach more people.",
      sections: [
        { label: "Approval", desc: "Admin reviews all new listings" },
        { label: "Nudge", desc: "Nudge admin after 3 days if pending" }
      ]
    }
  ];
    ...
    >
      ...
      >
        ...
        <div className="text-center space-y-4">
          <div className="w-16 h-16 bg-gray-50 dark:bg-gray-800 rounded-2xl flex items-center justify-center mx-auto shadow-sm">
            {current.icon}
          </div>
          <div className="space-y-1">
            <h2 className={`text-2xl font-black ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{current.title}</h2>
            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{current.content}</p>
          </div>
        </div>

        <div className="space-y-4">
          {current.sections.map((s, i) => (
            <div key={i} className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-gray-50 border-gray-100'}`}>
              <h4 className={`text-xs font-bold uppercase tracking-wider ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>{s.label}</h4>
              <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{s.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-3">
          {step > 0 && (
            <button
              onClick={() => setStep(s => s - 1)}
              className={`flex-1 py-4 rounded-2xl font-bold transition-all ${isDarkMode ? 'bg-gray-800 text-gray-300' : 'bg-gray-100 text-gray-600'}`}
            >
              Back
            </button>
          )}
          <button
            ...
            className="flex-[2] bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-2xl font-bold shadow-lg shadow-blue-500/20 transition-all active:scale-95"
          >
        ...
        <div className="flex justify-center gap-1.5">
          {steps.map((_, i) => (
            <div key={i} className={`h-1 rounded-full transition-all ${i === step ? 'w-6 bg-blue-600' : 'w-2 bg-gray-200 dark:bg-gray-700'}`} />
          ))}
        </div>
      </motion.div>
    </motion.div>
