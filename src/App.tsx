...
// Snap-Back Zoom Component
const SnapBackZoom = ({ children }: { children: React.ReactNode }) => {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (isZooming && e.touches.length === 2) {
      ...
      const newScale = (dist / startDist.current) * startScale.current;
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    if (e.button === 0 && e.shiftKey) { // Shift + Left Click to drag zoom
      setIsZooming(true);
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (isZooming) {
      const diff = startY.current - e.pageY;
      const newScale = startScale.current + diff / 300;
      setScale(Math.max(1, Math.min(3, newScale)));
    }
  };
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: isZooming ? 'none' : 'transform 0.6s cubic-bezier(0.34, 1.56, 0.64, 1)',
        width: '100%',
        minHeight: '100vh',
